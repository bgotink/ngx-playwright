...
import type {MainComponentHarness} from '../harnesses/app-component-harness';
import {
  SubComponentHarness,
...
export function crossEnvironmentSpecs(
  ...
  describe('HarnessLoader', () => {
    ...
    it('should find all HarnessLoaders for child elements', async () => {
      const loaders = await loader.getAllChildLoaders(
        '.subcomponents,.counters',
      );
  ...
  describe('HarnessPredicate', () => {
    ...
    it('should find subcomponents that match selector', async () => {
      ...
      expect(await (await lastList.title()).text()).toBe(
        'List of test methods',
      );
    });
  ...
  describe('TestElement', () => {
    ...
    it('should check if selector matches', async () => {
      const button = await harness.button();
      expect(await button.matchesSelector('button:not(.fake-class)')).toBe(
        true,
      );
...
export async function checkIsElement(
